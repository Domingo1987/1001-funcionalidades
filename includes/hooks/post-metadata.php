    ...
    if (!empty($categories)) {
        foreach ($categories as $category) {
            if ($category->name != 'Blog') {
                $output .= '<a href="https://1001problemas.com/problemas/' . esc_attr($category->slug) . '">' . esc_html($category->name) . '</a>, ';
            }
        }
    } else {
        ...
        if (!empty($ia_categories)) {
            foreach ($ia_categories as $ia_category) {
                if ($ia_category->name != 'Inteligencia Artificial') {
                    $output .= '<a href="https://1001problemas.com/ia/' . esc_attr($ia_category->slug) . '">' . esc_html($ia_category->name) . '</a>, ';
                }
            }
        }
    }
